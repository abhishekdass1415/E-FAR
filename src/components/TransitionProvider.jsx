...
export default function TransitionProvider({ children, carSrc = "/vite.svg", duration = 1.2, carHeight = 64 }) {
  ...
  const start = useCallback((to) => {
    ...
    setTarget(to);
    // Preload the image so the animation is visible immediately
    const img = new Image();
    img.src = carSrc;
    ...
    // Safety fallback if decode/onload doesn't fire promptly
    setTimeout(show, 120);
  }, [carSrc]);
        ...
        {active && (
          ...
          >
            ...
            <motion.img
              src={carSrc}
              alt="transition-car"
              className="absolute bottom-10 left-0 w-auto drop-shadow-[0_4px_8px_rgba(0,0,0,0.6)]"
            ...
            />
            {/* Subtle tire smoke puffs (EV-friendly) */}
            <motion.div
              className="absolute bottom-9 left-0 h-3 w-3 rounded-full bg-white/50 blur-[2px]"
              initial={{ x: "-30%", opacity: 0.7, scale: 1 }}
              animate={{ x: "100%", opacity: 0, scale: 2 }}
              transition={{ duration: Math.max(0.45, duration), ease: "easeInOut" }}
            />
            <motion.div
              className="absolute bottom-9 left-6 h-2 w-2 rounded-full bg-white/40 blur-[2px]"
